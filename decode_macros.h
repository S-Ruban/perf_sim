...
#define fsgnj16(a, b, n, x)                                       \
  f16((f16(a).v & ~F16_SIGN) | ((((x) ? f16(a).v : (n) ? F16_SIGN \
                                                       : 0) ^     \
                                 f16(b).v) &                      \
                                F16_SIGN))
#define fsgnj32(a, b, n, x)                                       \
  f32((f32(a).v & ~F32_SIGN) | ((((x) ? f32(a).v : (n) ? F32_SIGN \
                                                       : 0) ^     \
                                 f32(b).v) &                      \
                                F32_SIGN))
#define fsgnj64(a, b, n, x)                                       \
  f64((f64(a).v & ~F64_SIGN) | ((((x) ? f64(a).v : (n) ? F64_SIGN \
                                                       : 0) ^     \
                                 f64(b).v) &                      \
                                F64_SIGN))
...
{
  a.v[1] = (a.v[1] & ~F64_SIGN) | (((x ? a.v[1] : n ? F64_SIGN
                                                    : 0) ^
                                    b.v[1]) &
                                   F64_SIGN);
  return a;
...
inline float to_f(float32_t f)
{
  float r;
  memcpy(&r, &f, sizeof(r));
  return r;
}
inline double to_f(float64_t f)
{
  double r;
  memcpy(&r, &f, sizeof(r));
  return r;
}
inline long double to_f(float128_t f)
{
  long double r;
  memcpy(&r, &f, sizeof(r));
  return r;
