...
void throw_access_exception(bool virt, reg_t addr, access_type type)
{
  ...
  case LOAD:
    throw trap_load_access_fault(virt, addr, 0, 0);
  case STORE:
...
bool mmu_t::mmio(reg_t paddr, size_t len, uint8_t *bytes, access_type type)
{
  ...
  {
    // fprintf(stderr, "napot\n");
    if (!mmio_ok(paddr, type))
    {
      // fprintf(stderr, "!mmio_ok()\n");
      return false;
    }
    // fprintf(stderr, "mmio_ok()\n");
...
void mmu_t::store_slow_path_intrapage(reg_t len, const uint8_t *bytes, mem_access_info_t access_info, bool actually_store)
{
  ...
  if (actually_store)
  {
    ...
    // else
    //   fprintf(stderr, "NULL\n");
    // fprintf(stderr, "paddr after translate: %x\n", paddr);
    if (auto host_addr = sim->addr_to_mem(paddr))
    {
      memcpy(host_addr, bytes, len);
...
reg_t mmu_t::s2xlate(reg_t gva, reg_t gpa, access_type type, access_type trap_type, bool virt, bool hlvx, bool is_for_vs_pt_addr)
{
  ...
  if ((gpa & ~maxgpa) == 0)
  {
    for (int i = vm.levels - 1; i >= 0; i--)
    {
      ...
      else if (type == FETCH || hlvx ? !(pte & PTE_X) : type == LOAD ? !(pte & PTE_R) && !(mxr && (pte & PTE_X))
                                                                     : !((pte & PTE_R) && (pte & PTE_W)))
      {
        break;
      }
      ...
      else
      {
        ...
        reg_t page_base = ((ppn & ~((reg_t(1) << napot_bits) - 1)) | (vpn & ((reg_t(1) << napot_bits) - 1)) | (vpn & ((reg_t(1) << ptshift) - 1))) << PGSHIFT;
