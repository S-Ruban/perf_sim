...
#define convert_load_traps_to_store_traps(BODY)                                                   \
  ...
  template <typename T>
  inline T from_target(target_endian<T> n) const
  {
    return target_big_endian ? n.from_be() : n.from_le();
  }

  template <typename T>
  inline target_endian<T> to_target(T n) const
  {
...
private:
  ...
  template <typename T>
  inline reg_t pte_load(reg_t pte_paddr, reg_t addr, bool virt, access_type trap_type)
  {
  ...
  template <typename T>
  inline void pte_store(reg_t pte_paddr, reg_t new_pte, reg_t addr, bool virt, access_type trap_type)
  {
    const size_t ptesize = sizeof(T);
  ...
  inline bool in_mprv()
  {
    return proc != nullptr && !(proc->state.mnstatus && !get_field(proc->state.mnstatus->read(), MNSTATUS_NMIE)) && !proc->state.debug_mode && get_field(proc->state.mstatus->read(), MSTATUS_MPRV);
  }
...
inline vm_info decode_vm_info(int xlen, bool stage2, reg_t prv, reg_t satp)
{
  ...
  else if (!stage2 && prv <= PRV_S && xlen == 32)
  {
    ...
    case SATP_MODE_OFF:
      return {0, 0, 0, 0, 0};
    case SATP_MODE_SV32:
      return {2, 10, 0, 4, (satp & SATP32_PPN) << PGSHIFT};
    default:
      abort();
    }
  }
  else if (!stage2 && prv <= PRV_S && xlen == 64)
  {
    ...
    case SATP_MODE_OFF:
      return {0, 0, 0, 0, 0};
    case SATP_MODE_SV39:
      return {3, 9, 0, 8, (satp & SATP64_PPN) << PGSHIFT};
    case SATP_MODE_SV48:
      return {4, 9, 0, 8, (satp & SATP64_PPN) << PGSHIFT};
    case SATP_MODE_SV57:
      return {5, 9, 0, 8, (satp & SATP64_PPN) << PGSHIFT};
    case SATP_MODE_SV64:
      return {6, 9, 0, 8, (satp & SATP64_PPN) << PGSHIFT};
    default:
      abort();
    }
  }
  else if (stage2 && xlen == 32)
  {
    ...
    case HGATP_MODE_SV32X4:
      return {2, 10, 2, 4, (satp & HGATP32_PPN) << PGSHIFT};
    default:
      abort();
    }
  }
  else if (stage2 && xlen == 64)
  {
    ...
    case HGATP_MODE_SV39X4:
      return {3, 9, 2, 8, (satp & HGATP64_PPN) << PGSHIFT};
    case HGATP_MODE_SV48X4:
      return {4, 9, 2, 8, (satp & HGATP64_PPN) << PGSHIFT};
    case HGATP_MODE_SV57X4:
      return {5, 9, 2, 8, (satp & HGATP64_PPN) << PGSHIFT};
    default:
      abort();
    }
  }
