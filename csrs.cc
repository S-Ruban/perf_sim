...
csr_t::csr_t(processor_t *const proc, const reg_t addr) : proc(proc),
...
basic_csr_t::basic_csr_t(processor_t *const proc, const reg_t addr, const reg_t init) : csr_t(proc, addr),
                                                                                        val(init)
{
}
...
pmpaddr_csr_t::pmpaddr_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr),
                                                                          val(0),
                                                                          cfg(0),
...
{
  if (pmpidx == 0)
    return 0; // entry 0 always uses 0 as base
...
pmpcfg_csr_t::pmpcfg_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr)
{
}
...
mseccfg_csr_t::mseccfg_csr_t(processor_t *const proc, const reg_t addr) : basic_csr_t(proc, addr, 0)
{
}
...
virtualized_csr_t::virtualized_csr_t(processor_t *const proc, csr_t_p orig, csr_t_p virt) : csr_t(proc, orig->address),
                                                                                            orig_csr(orig),
                                                                                            virt_csr(virt)
{
}
...
epc_csr_t::epc_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr),
                                                                  val(0)
{
}
...
tvec_csr_t::tvec_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr),
                                                                    val(0)
{
}
...
cause_csr_t::cause_csr_t(processor_t *const proc, const reg_t addr) : basic_csr_t(proc, addr, 0)
{
}
...
base_status_csr_t::base_status_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr),
                                                                                  has_page(proc->extension_enabled_const('S') && proc->supports_impl(IMPL_MMU)),
                                                                                  sstatus_write_mask(compute_sstatus_write_mask()),
                                                                                  sstatus_read_mask(sstatus_write_mask | SSTATUS_UBE | SSTATUS_UXL | (proc->get_const_xlen() == 32 ? SSTATUS32_SD : SSTATUS64_SD))
{
}
...
reg_t base_status_csr_t::compute_sstatus_write_mask() const noexcept
{
  ...
  const bool has_fs = (proc->extension_enabled('S') || proc->extension_enabled('F') || proc->extension_enabled('V')) && !proc->extension_enabled(EXT_ZFINX);
  const bool has_vs = proc->extension_enabled('V');
  return 0 | (proc->extension_enabled('S') ? (SSTATUS_SIE | SSTATUS_SPIE | SSTATUS_SPP) : 0) | (has_page ? (SSTATUS_SUM | SSTATUS_MXR) : 0) | (has_fs ? SSTATUS_FS : 0) | (proc->any_custom_extensions() ? SSTATUS_XS : 0) | (has_vs ? SSTATUS_VS : 0);
}
...
{
  if ((newval ^ read()) &
      (MSTATUS_MPP | MSTATUS_MPRV | (has_page ? (MSTATUS_MXR | MSTATUS_SUM) : 0)))
    proc->get_mmu()->flush_tlb();
}
...
vsstatus_csr_t::vsstatus_csr_t(processor_t *const proc, const reg_t addr) : base_status_csr_t(proc, addr),
...
bool vsstatus_csr_t::unlogged_write(const reg_t val) noexcept
{
  ...
  if (state->v)
    maybe_flush_tlb(newval);
  this->val = adjust_sd(newval);
  return true;
}

// implement class sstatus_proxy_csr_t
sstatus_proxy_csr_t::sstatus_proxy_csr_t(processor_t *const proc, const reg_t addr, mstatus_csr_t_p mstatus) : base_status_csr_t(proc, addr),
                                                                                                               mstatus(mstatus)
{
}
...
mstatus_csr_t::mstatus_csr_t(processor_t *const proc, const reg_t addr) : base_status_csr_t(proc, addr),
...
bool mstatus_csr_t::unlogged_write(const reg_t val) noexcept
{
  ...
  const reg_t mask = sstatus_write_mask | MSTATUS_MIE | MSTATUS_MPIE | (proc->extension_enabled('U') ? MSTATUS_MPRV : 0) | MSTATUS_MPP | MSTATUS_TW | (proc->extension_enabled('S') ? MSTATUS_TSR : 0) | (has_page ? MSTATUS_TVM : 0) | (has_gva ? MSTATUS_GVA : 0) | (has_mpv ? MSTATUS_MPV : 0);
...
{
  const reg_t big_endian_bits = (proc->extension_enabled_const('U') ? MSTATUS_UBE : 0) | (proc->extension_enabled_const('S') ? MSTATUS_SBE : 0) | MSTATUS_MBE;
  return 0 | set_field((reg_t)0, MSTATUS_MPP, proc->extension_enabled_const('U') ? PRV_U : PRV_M) | (proc->extension_enabled_const('U') && (proc->get_const_xlen() != 32) ? set_field((reg_t)0, MSTATUS_UXL, xlen_to_uxl(proc->get_const_xlen())) : 0) | (proc->extension_enabled_const('S') && (proc->get_const_xlen() != 32) ? set_field((reg_t)0, MSTATUS_SXL, xlen_to_uxl(proc->get_const_xlen())) : 0) | (proc->get_mmu()->is_target_big_endian() ? big_endian_bits : 0) | 0; // initial value for mstatus
}

// implement class mnstatus_csr_t
mnstatus_csr_t::mnstatus_csr_t(processor_t *const proc, const reg_t addr) : basic_csr_t(proc, addr, 0)
{
}
...
bool mnstatus_csr_t::unlogged_write(const reg_t val) noexcept
{
  ...
  const reg_t mask = (~read() & MNSTATUS_NMIE) | (proc->extension_enabled('H') ? MNSTATUS_MNPV : 0) | MNSTATUS_MNPP;
...
rv32_low_csr_t::rv32_low_csr_t(processor_t *const proc, const reg_t addr, csr_t_p orig) : csr_t(proc, addr),
                                                                                          orig(orig)
{
}
...
rv32_high_csr_t::rv32_high_csr_t(processor_t *const proc, const reg_t addr, csr_t_p orig) : csr_t(proc, addr),
                                                                                            orig(orig)
{
}
...
sstatus_csr_t::sstatus_csr_t(processor_t *const proc, sstatus_proxy_csr_t_p orig, vsstatus_csr_t_p virt) : virtualized_csr_t(proc, orig, virt),
                                                                                                           orig_sstatus(orig),
                                                                                                           virt_sstatus(virt)
{
}
...
misa_csr_t::misa_csr_t(processor_t *const proc, const reg_t addr, const reg_t max_isa) : basic_csr_t(proc, addr, max_isa),
                                                                                         max_isa(max_isa),
                                                                                         write_mask(max_isa & (0 // allow MABFDQCHV bits in MISA to be modified
                                                                                                               | (1L << ('M' - 'A')) | (1L << ('A' - 'A')) | (1L << ('B' - 'A')) | (1L << ('F' - 'A')) | (1L << ('D' - 'A')) | (1L << ('Q' - 'A')) | (1L << ('C' - 'A')) | (1L << ('H' - 'A')) | (1L << ('V' - 'A'))))
{
}
...
bool misa_csr_t::unlogged_write(const reg_t val) noexcept
{
  ...
  if (!new_h && prev_h)
  {
    reg_t hypervisor_exceptions = 0 | (1 << CAUSE_VIRTUAL_SUPERVISOR_ECALL) | (1 << CAUSE_FETCH_GUEST_PAGE_FAULT) | (1 << CAUSE_LOAD_GUEST_PAGE_FAULT) | (1 << CAUSE_VIRTUAL_INSTRUCTION) | (1 << CAUSE_STORE_GUEST_PAGE_FAULT);

    state->medeleg->write(state->medeleg->read() & ~hypervisor_exceptions);
    if (state->mnstatus)
      state->mnstatus->write(state->mnstatus->read() & ~MNSTATUS_MNPV);
    const reg_t new_mstatus = state->mstatus->read() & ~(MSTATUS_GVA | MSTATUS_MPV);
    state->mstatus->write(new_mstatus);
    if (state->mstatush)
      state->mstatush->write(new_mstatus >> 32); // log mstatush change
...
mip_or_mie_csr_t::mip_or_mie_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr),
                                                                                val(0)
{
}
...
mip_csr_t::mip_csr_t(processor_t *const proc, const reg_t addr) : mip_or_mie_csr_t(proc, addr)
{
}
...
mie_csr_t::mie_csr_t(processor_t *const proc, const reg_t addr) : mip_or_mie_csr_t(proc, addr)
{
}
...
generic_int_accessor_t::generic_int_accessor_t(state_t *const state,
                                               ...
                                               const int shiftamt) : state(state),
                                                                     read_mask(read_mask),
...
mip_proxy_csr_t::mip_proxy_csr_t(processor_t *const proc, const reg_t addr, generic_int_accessor_t_p accr) : csr_t(proc, addr),
                                                                                                             accr(accr)
{
}
...
mie_proxy_csr_t::mie_proxy_csr_t(processor_t *const proc, const reg_t addr, generic_int_accessor_t_p accr) : csr_t(proc, addr),
                                                                                                             accr(accr)
{
}
...
mideleg_csr_t::mideleg_csr_t(processor_t *const proc, const reg_t addr) : basic_csr_t(proc, addr, 0)
{
}
...
reg_t mideleg_csr_t::read() const noexcept
{
  ...
  if (proc->extension_enabled('H'))
    return val | MIDELEG_FORCED_MASK;
...
medeleg_csr_t::medeleg_csr_t(processor_t *const proc, const reg_t addr) : basic_csr_t(proc, addr, 0),
                                                                          hypervisor_exceptions(0 | (1 << CAUSE_VIRTUAL_SUPERVISOR_ECALL) | (1 << CAUSE_FETCH_GUEST_PAGE_FAULT) | (1 << CAUSE_LOAD_GUEST_PAGE_FAULT) | (1 << CAUSE_VIRTUAL_INSTRUCTION) | (1 << CAUSE_STORE_GUEST_PAGE_FAULT))
{
}
...
{
  const reg_t mask = 0 | (1 << CAUSE_MISALIGNED_FETCH) | (1 << CAUSE_FETCH_ACCESS) | (1 << CAUSE_ILLEGAL_INSTRUCTION) | (1 << CAUSE_BREAKPOINT) | (1 << CAUSE_MISALIGNED_LOAD) | (1 << CAUSE_LOAD_ACCESS) | (1 << CAUSE_MISALIGNED_STORE) | (1 << CAUSE_STORE_ACCESS) | (1 << CAUSE_USER_ECALL) | (1 << CAUSE_SUPERVISOR_ECALL) | (1 << CAUSE_FETCH_PAGE_FAULT) | (1 << CAUSE_LOAD_PAGE_FAULT) | (1 << CAUSE_STORE_PAGE_FAULT) | (proc->extension_enabled('H') ? hypervisor_exceptions : 0);
  return basic_csr_t::unlogged_write((read() & ~mask) | (val & mask));
}

// implement class masked_csr_t
masked_csr_t::masked_csr_t(processor_t *const proc, const reg_t addr, const reg_t mask, const reg_t init) : basic_csr_t(proc, addr, init),
                                                                                                            mask(mask)
{
}
...
envcfg_csr_t::envcfg_csr_t(processor_t *const proc, const reg_t addr, const reg_t mask,
                           const reg_t init) : masked_csr_t(proc, addr, mask, init)
{
...
henvcfg_csr_t::henvcfg_csr_t(processor_t *const proc, const reg_t addr, const reg_t mask, const reg_t init, csr_t_p menvcfg) : envcfg_csr_t(proc, addr, mask, init),
                                                                                                                               menvcfg(menvcfg)
{
}

// implement class base_atp_csr_t and family
base_atp_csr_t::base_atp_csr_t(processor_t *const proc, const reg_t addr) : basic_csr_t(proc, addr, 0)
{
}
...
bool base_atp_csr_t::satp_valid(reg_t val) const noexcept
{
  if (proc->get_xlen() == 32)
  {
    ...
    {
    case SATP_MODE_SV32:
      return proc->supports_impl(IMPL_MMU_SV32);
    case SATP_MODE_OFF:
      return true;
    default:
      return false;
  ...
  else
  {
    ...
    {
    case SATP_MODE_SV39:
      return proc->supports_impl(IMPL_MMU_SV39);
    case SATP_MODE_SV48:
      return proc->supports_impl(IMPL_MMU_SV48);
    case SATP_MODE_SV57:
      return proc->supports_impl(IMPL_MMU_SV57);
    case SATP_MODE_OFF:
      return true;
    default:
      return false;
...
satp_csr_t::satp_csr_t(processor_t *const proc, const reg_t addr) : base_atp_csr_t(proc, addr)
{
}
...
virtualized_satp_csr_t::virtualized_satp_csr_t(processor_t *const proc, satp_csr_t_p orig, csr_t_p virt) : virtualized_csr_t(proc, orig, virt),
                                                                                                           orig_satp(orig)
{
}
...
wide_counter_csr_t::wide_counter_csr_t(processor_t *const proc, const reg_t addr, smcntrpmf_csr_t_p config_csr) : csr_t(proc, addr),
                                                                                                                  val(0),
...
time_counter_csr_t::time_counter_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr),
                                                                                    shadow_val(0)
{
}
...
proxy_csr_t::proxy_csr_t(processor_t *const proc, const reg_t addr, csr_t_p delegate) : csr_t(proc, addr),
                                                                                        delegate(delegate)
{
}
...
const_csr_t::const_csr_t(processor_t *const proc, const reg_t addr, reg_t val) : csr_t(proc, addr),
                                                                                 val(val)
{
}
...
counter_proxy_csr_t::counter_proxy_csr_t(processor_t *const proc, const reg_t addr, csr_t_p delegate) : proxy_csr_t(proc, addr, delegate)
{
}
...
mevent_csr_t::mevent_csr_t(processor_t *const proc, const reg_t addr) : basic_csr_t(proc, addr, 0)
{
}

bool mevent_csr_t::unlogged_write(const reg_t val) noexcept
{
  const reg_t mask = proc->extension_enabled(EXT_SSCOFPMF) ? MHPMEVENT_OF | MHPMEVENT_MINH | (proc->extension_enabled_const('U') ? MHPMEVENT_UINH : 0) | (proc->extension_enabled_const('S') ? MHPMEVENT_SINH : 0) | (proc->extension_enabled('H') ? MHPMEVENT_VUINH | MHPMEVENT_VSINH : 0) : 0;
  return basic_csr_t::unlogged_write((read() & ~mask) | (val & mask));
}

hypervisor_csr_t::hypervisor_csr_t(processor_t *const proc, const reg_t addr) : basic_csr_t(proc, addr, 0)
{
}
...
hideleg_csr_t::hideleg_csr_t(processor_t *const proc, const reg_t addr, csr_t_p mideleg) : masked_csr_t(proc, addr, MIP_VS_MASK, 0),
                                                                                           mideleg(mideleg)
{
}
...
hgatp_csr_t::hgatp_csr_t(processor_t *const proc, const reg_t addr) : basic_csr_t(proc, addr, 0)
{
}
...
tselect_csr_t::tselect_csr_t(processor_t *const proc, const reg_t addr) : basic_csr_t(proc, addr, 0)
{
}
...
tdata1_csr_t::tdata1_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr)
{
}
...
tdata2_csr_t::tdata2_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr)
{
}
...
tdata3_csr_t::tdata3_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr)
{
}
...
tinfo_csr_t::tinfo_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr)
{
}
...
debug_mode_csr_t::debug_mode_csr_t(processor_t *const proc, const reg_t addr) : basic_csr_t(proc, addr, 0)
{
}
...
dpc_csr_t::dpc_csr_t(processor_t *const proc, const reg_t addr) : epc_csr_t(proc, addr)
{
}
...
dcsr_csr_t::dcsr_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr),
                                                                    prv(0),
                                                                    step(false),
                                                                    ebreakm(false),
                                                                    ebreaks(false),
...
float_csr_t::float_csr_t(processor_t *const proc, const reg_t addr, const reg_t mask, const reg_t init) : masked_csr_t(proc, addr, mask, init)
{
}
...
composite_csr_t::composite_csr_t(processor_t *const proc, const reg_t addr, csr_t_p upper_csr, csr_t_p lower_csr, const unsigned upper_lsb) : csr_t(proc, addr),
                                                                                                                                              upper_csr(upper_csr),
                                                                                                                                              lower_csr(lower_csr),
                                                                                                                                              upper_lsb(upper_lsb)
{
...
seed_csr_t::seed_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr)
{
}
...
vector_csr_t::vector_csr_t(processor_t *const proc, const reg_t addr, const reg_t mask, const reg_t init) : basic_csr_t(proc, addr, init),
                                                                                                            mask(mask)
{
}
...
vxsat_csr_t::vxsat_csr_t(processor_t *const proc, const reg_t addr) : masked_csr_t(proc, addr, /*mask*/ 1, /*init*/ 0)
{
}
...
hstateen_csr_t::hstateen_csr_t(processor_t *const proc, const reg_t addr, const reg_t mask,
                               const reg_t init, uint8_t index) : masked_csr_t(proc, addr, mask, init),
                                                                  index(index)
{
}
...
sstateen_csr_t::sstateen_csr_t(processor_t *const proc, const reg_t addr, const reg_t mask,
                               const reg_t init, uint8_t index) : hstateen_csr_t(proc, addr, mask, init, index)
{
}
...
senvcfg_csr_t::senvcfg_csr_t(processor_t *const proc, const reg_t addr, const reg_t mask,
                             const reg_t init) : envcfg_csr_t(proc, addr, mask, init)
{
}
...
stimecmp_csr_t::stimecmp_csr_t(processor_t *const proc, const reg_t addr, const reg_t imask) : basic_csr_t(proc, addr, 0), intr_mask(imask)
{
}
...
virtualized_stimecmp_csr_t::virtualized_stimecmp_csr_t(processor_t *const proc, csr_t_p orig, csr_t_p virt) : virtualized_csr_t(proc, orig, virt)
{
}
...
scountovf_csr_t::scountovf_csr_t(processor_t *const proc, const reg_t addr) : csr_t(proc, addr)
{
}
...
jvt_csr_t::jvt_csr_t(processor_t *const proc, const reg_t addr, const reg_t init) : basic_csr_t(proc, addr, init)
{
}
...
virtualized_indirect_csr_t::virtualized_indirect_csr_t(processor_t *const proc, csr_t_p orig, csr_t_p virt) : virtualized_csr_t(proc, orig, virt)
{
}
...
sscsrind_reg_csr_t::sscsrind_reg_csr_t(processor_t *const proc, const reg_t addr, csr_t_p iselect) : csr_t(proc, addr),
                                                                                                     iselect(iselect)
{
}
...
srmcfg_csr_t::srmcfg_csr_t(processor_t *const proc, const reg_t addr, const reg_t mask, const reg_t init) : masked_csr_t(proc, addr, mask, init)
{
}
...
hvip_csr_t::hvip_csr_t(processor_t *const proc, const reg_t addr, const reg_t init) : basic_csr_t(proc, addr, init)
{
}
