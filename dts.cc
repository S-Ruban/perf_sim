                     ...
                     std::string device_nodes)
{
  ...
  for (size_t i = 0; i < procs.size(); i++)
  {
    s << "    CPU" << i << ": cpu@" << i << " {\n"
      ...
      << i << ">;\n"
              ...
              "      compatible = \"riscv\";\n"
              "      riscv,isa = \""
      << procs[i]->get_isa().get_isa_string() << "\";\n"
                                                 "      mmu-type = \"riscv,"
      << (procs[i]->get_isa().get_max_xlen() <= 32 ? "sv32" : "sv57") << "\";\n"
                                                                         "      riscv,pmpregions = <"
      << pmpregions << ">;\n"
  ...
  for (auto &m : mems)
  {
    s << std::hex << "  memory@" << m.first << " {\n"
                                               "    device_type = \"memory\";\n"
                                               "    reg = <0x"
      << (m.first >> 32) << " 0x" << (m.first & (uint32_t)-1) << " 0x" << (m.second->size() >> 16 >> 16) << " 0x" << (m.second->size() & (uint32_t)-1) << ">;\n"
                                                                                                                                                          "  };\n";
  }
  s << "  soc {\n"
       "    #address-cells = <2>;\n"
