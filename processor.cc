...
void state_t::reset(processor_t *const proc, reg_t max_isa)
{
  ...
  const reg_t mask = smcntrpmf_enabled ? MHPMEVENT_MINH | MHPMEVENT_SINH |
                                             MHPMEVENT_UINH | MHPMEVENT_VSINH | MHPMEVENT_VUINH
                                       : 0;
  ...
  if (proc->extension_enabled_const('U'))
    csrmap[CSR_MCOUNTEREN] = mcounteren;
  ...
  const reg_t hstatus_mask = HSTATUS_VTSR | HSTATUS_VTW | (proc->supports_impl(IMPL_MMU) ? HSTATUS_VTVM : 0) | HSTATUS_HU | HSTATUS_SPVP | HSTATUS_SPV | HSTATUS_GVA;
...
extension_t *processor_t::get_extension()
{
  ...
  case 0:
    return NULL;
  case 1:
    return custom_extensions.begin()->second;
  default:
...
void processor_t::take_interrupt(reg_t pending_interrupts)
{
  ...
  if (!state.debug_mode && nmie && enabled_interrupts)
  {
    ...
    if (check_triggers_icount)
      TM.detect_icount_match();
...
#define DECLARE_OVERLAP_INSN(name, ext)             \
  {                                                 \
    name##_supported = isa->extension_enabled(ext); \
  }
